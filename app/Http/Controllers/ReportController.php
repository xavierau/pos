...
class ReportController extends BaseController
{
    ...
    public function Returns_Client(request $request)
    {
        ...
        foreach ($SaleReturn as $Sale_Return) {
            ...
            $item['status'] = $Sale_Return->status;
    ...
    public function Returns_Provider(request $request)
    {
        ...
        foreach ($PurchaseReturn as $Purchase_Return) {
            ...
            $item['status'] = $Purchase_Return->status;
    ...
    public function count_quantity_alert(request $request)
    {
     ...
     public function ProfitAndLoss(request $request)
     {
         ...
         //--------Payment_sales
         $report_total_Payment_sales = PaymentSale::with('sale')
         ->where('deleted_at', '=', null)
         ->whereBetween('date', array($start_date, $end_date))
         ...
         $item['Payment_sales'] =   number_format($report_total_Payment_sales->sum, 2, '.', ',');
        ...
        //--------PaymentPurchaseReturns
         ...
         //--------Payment_purchases
         $report_total_Payment_purchases = PaymentPurchase::with('purchase')
         ->where('deleted_at', '=', null)
         ->whereBetween('date', array($start_date, $end_date))
         ...
         $item['Payment_purchases'] =   number_format($report_total_Payment_purchases->sum, 2, '.', ',');
      ...
      //calcule COGS and average cost
         ...
         $item['payment_received'] = number_format($report_total_Payment_sales->sum  + $report_total_PaymentPurchaseReturns->sum, 2, '.', ',');
         $item['payment_sent'] = number_format($report_total_Payment_purchases->sum + $report_total_PaymentSaleReturns->sum + $report_total_expenses->sum, 2, '.', ',');
         $item['Payment_net'] = number_format(($report_total_Payment_sales->sum  + $report_total_PaymentPurchaseReturns->sum)-($report_total_Payment_purchases->sum + $report_total_PaymentSaleReturns->sum + $report_total_expenses->sum), 2, '.', ',');
        ...
        // Get all distinct product IDs for sales between start and end date
         ...
         ->where(function ($query) use ($warehouse_id, $array_warehouses_id) {
             if ($warehouse_id !== 0) {
                 return $query->whereHas('sale', function ($q) use ($array_warehouses_id, $warehouse_id) {
                     $q->where('warehouse_id', $warehouse_id)->where('status', 'completed');
                 });
             }else{
                 return $query->whereHas('sale', function ($q) use ($array_warehouses_id, $warehouse_id) {
                     $q->whereIn('warehouse_id', $array_warehouses_id)->where('status', 'completed');
                 });
         ...
         foreach ($productIds as $productId) {
             ...
             ->sum('quantity');
              ...
              ->where(function ($query) use ($warehouse_id, $array_warehouses_id) {
                 if ($warehouse_id !== 0) {
                     return  $query->where('purchases.warehouse_id', $warehouse_id)->where('purchases.status', 'received');
                 }else{
                     return  $query->whereIn('purchases.warehouse_id', $array_warehouses_id)->where('purchases.status', 'received');
     ...
     public function download_report_provider_pdf(Request $request, $id)
     {
        ...
        $this->authorizeForUser($request->user('api'), 'Reports_suppliers', Provider::class);
         ...
         foreach ($purchases as $purchase) {
             ...
             $item_purchase['payment_status'] = $purchase['payment_status'];
