...
class UpdatePromotionRequest extends FormRequest
{
    ...
    {
        $rules = [
            'id' => ['required', Rule::exists('promotions', 'id')],
        ...
        if ($type === PromotionType::BuyXGetY) {
            $rules = array_merge($rules, [
                ...
                'rule.x_product.product_variant_id' => [Rule::exists('product_variants', 'id')->where(fn($q) => $q->where('product_id', $this->get('rule')['x_product']['product_id']))],
                'rule.y_product' => ['required'],
                'rule.y_product.product_id' => ['required', Rule::exists('products', 'id')],
                'rule.y_product.product_variant_id' => [Rule::exists('product_variants', 'id')->where(fn($q) => $q->where('product_id', $this->get('rule')['y_product']['product_id']))],
            ]);
