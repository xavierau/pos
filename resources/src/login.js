...
axios.interceptors.response.use((response) => {
    ...
    if (error.response && error.response.data) {
        if (error.response.status === 401) {
            // console.log("in login js, error 401", error, window.location.pathname, window.location.pathname !== '/simple_login');
            if (window.location.pathname !== '/simple_login') {
...
Vue.component('simple-login-component', require('./views/app/sessions/simple_signIn.vue').default);
