              ...
              >
                ...
                >
                 ...
                 <b-dropdown-item
                     ...
                     v-if="currentUserPermissions.includes('Purchase_Returns_add') && props.row.purchase_has_return == 'no' && props.row.status == 'received'"
                ...
                <b-dropdown-item
                    v-if="currentUserPermissions.includes('payment_purchases_add') && props.row.status == 'received'"
        ...
        <b-modal hide-footer size="lg" id="Show_payment" :title="$t('ShowPayment')">
            <b-row>
                <b-col lg="12" md="12" sm="12" class="mt-3">
                    <PaymentsTable :payments="factures"/>
                ...
                <b-form @submit.prevent="submitPayment">
                    <b-row>
                        ...
                        <b-col lg="4" md="12" sm="12">
                            ...
                            >
                                <b-form-group :label="$t('Paying_Amount')">
                                    <b-form-input
                                        @keyup="Verified_paidAmount(facture.amount)"
                                        label="Amount"
                                        :placeholder="$t('Paying_Amount')"
                                        v-model.number="facture.amount"
                                        :state="getValidationState(validationContext)"
                        ...
                        <b-col lg="4" md="12" sm="12">
                            ...
                            <p
                                class="change_amount"
                            >{{ parseFloat(facture.received_amount - facture.amount).toFixed(2) }}</p>
                        </b-col>
                        ...
                        <b-col lg="6" md="6" sm="12">
                            <b-form-group :label="$t('Note')">
                                <b-form-textarea id="textarea" v-model="facture.notes" rows="3"
...
import PaymentsTable from "../../../../components/purchase/PaymentsTable";
...
export default {
    ...
    data() {
        return {
            registerEvents: {
                [PurchasePaymentEvent.EditPayment]: (payment) => this.Edit_Payment(payment),
                [PurchasePaymentEvent.Pdf]: (payment) => this.Payment_Purchase_PDF(payment),
                [PurchasePaymentEvent.SendPaymentEmail]: (payment) => this.Send_Email_Payment(payment),
                [PurchasePaymentEvent.SendPaymentSMS]: (payment) => this.Payment_Purchase_SMS(payment),
                [PurchasePaymentEvent.RemovePayment]: (payment) => this.Remove_Payment(payment),
            },
    ...
    computed: {
        ...mapGetters(["supportedPaymentTypes"]),
        columns() {
            return [
                {
                    label: this.$t("date"),
    ...
    methods: {
        ...
        //---------SMS notification
        Payment_Purchase_SMS(payment) {
            this.execute(() => {
                // Start the progress bar.
                axios.post("payment_purchase_send_sms", {id: payment.id,})
                    .then(response => this.makeSuccessToast("Send_SMS"))
                    .catch(error => {
                        console.error('Payment_Purchase_SMS: ', error);
                        this.makeDangerToast("sms_config_invalid", "Failed");
                    });
            })
        },


        Send_Email_Payment(payment) {
            this.execute(() => {
                axios.post("payment_purchase_send_email", {id: payment.id,})
                    .then(response => this.makeSuccessToast("Send.TitleEmail"))
                    .catch(error => {
                        console.error('Send_Email_Payment: ', error);
                        this.makeDangerToast("SMTPIncorrect", "Failed");
                    });
            })
        },
        ...
        Number_Order_Payment() {
            axios.get("payment_purchase_get_number")
                .then(({data}) => {
                    console.log('new ref: ', data, data.ref)
                    this.facture.ref = data.ref
                });
        },
        ...
        Remove_Payment(payment) {
            this.$swal({
                ...
                if (result.value) {
                    this.execute(() => {
                        axios.delete("payment_purchase/" + payment.id)
                            .then(() => {
                                this.makeSuccessToast("Delete.PaymentDeleted")
                                Fire.$emit("Delete_Facture_purchase");
                            })
                            .catch((error) => {
                                console.error("Remove_Payment: ", error)
                                this.makeWarningToast("Delete.Therewassomethingwronge", "Delete.Failed")
                            });
                    })
